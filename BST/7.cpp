// 🌳 Question: Kth Largest Element in a BST (Brute Force + Optimized Solutions)
...
// ---------------------------------------------------------
// 🧩 Solution 1: Brute Force (Store Reverse Inorder Traversal in a Vector)
// ---------------------------------------------------------
...
public:
    void reverseInorder(TreeNode* root, int& ans, int& k) {
        if (!root) return;

        reverseInorder(root->right, ans, k);
        k--;
        if (k == 0) {
            ans = root->val;
            return;
        }
        reverseInorder(root->left, ans, k);
    }

    int kthLargest(TreeNode* root, int k) {
        int ans = 0;
        reverseInorder(root, ans, k);
        return ans;
    }
};
...
int main() {
    ...
    cout << "Brute Force → " << brute.kthLargest(root, k) << "\n";
    cout << "Optimized (Count) → " << optCount.kthLargest(root, k) << "\n";
    cout << "Optimized (Decrement) → " << optDec.kthLargest(root, k) << "\n";
